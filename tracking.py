...
class ParticleTracker:
    def __init__(self, max_disappeared=10, max_distance=30):
        ...
        self.object_info = {} # Stores shape and other info
    ...
    def update(self, detected_particles, frame_height, frame_width):
        """
        Update tracker with new detections for the current frame.
        `detected_particles` is a list of dicts from `analyze_particles`.
        """
        ...
        for i, p in enumerate(detected_particles):
            # --- CORRECTION: Use dynamic frame dimensions ---
...
def main():
    ...
    for frame_idx in tqdm(range(total_frames), desc="Tracking Particles"):
        ...
        # --- CORRECTION: Pass frame dimensions to the tracker ---
        tracked_objects = tracker.update(particles_stats, height, width)
